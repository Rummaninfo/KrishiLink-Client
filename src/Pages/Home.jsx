        ...
        {slides.map((s, i) => (
          ...
          >
            ...
            <div className="absolute left-6 bottom-8 max-w-xl text-white">
              ...
              <p className="mt-2 text-sm md:text-base text-white/90">{s.subtitle}</p>
              {s.cta && (
                <Link
                  to={s.cta.href}
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
        {slides.map((_, i) => (
          <button
            ...
            className={`h-2 w-8 rounded-full transition ${i === idx ? "bg-emerald-600" : "bg-white/60"}`}
...
const HowItWorks = ({ className = "" }) => {
  ...
  return (
    <section className={`mt-12 p-6 bg-white rounded-2xl shadow-sm ${className}`}>
      <div className="max-w-4xl mx-auto">
        <h3 className="text-2xl font-semibold text-center text-emerald-700">How it works</h3>
        <p className="text-sm text-center text-slate-500 mt-2">Simple steps to buy or sell fresh crops.</p>
          ...
          {steps.map((s, i) => (
            ...
            >
              ...
              <div className="relative z-10">
                <div className="inline-flex items-center justify-center h-9 w-9 rounded-full bg-emerald-600 text-white font-semibold mb-3">{i + 1}</div>
                <div className="font-semibold text-slate-800">{s.title}</div>
                <div className="text-xs text-slate-600 mt-1 leading-snug">{s.desc}</div>
              </div>
            </motion.div>
          ))}
        </motion.div>
...
const AgroNews = () => {
  ...
  return (
    <section className="mt-16 bg-[#fef6e2] py-12">
      <div className="max-w-6xl mx-auto px-4 text-center">
        <h2 className="text-3xl md:text-4xl font-semibold text-emerald-600">Harvesting Insights</h2>
        <p className="text-slate-600 mt-2">Explore our latest thinking on sustainable farming</p>

        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {posts.map((p) => (
            <article key={p.id} className="text-left">
              <img src={p.image} alt={p.title} className="w-full h-44 object-cover rounded-xl shadow-sm" />
              <h3 className="mt-4 text-base md:text-lg font-semibold text-slate-900 leading-snug">{p.title}</h3>
    ...
    <section className="relative w-full h-[420px] lg:h-[520px] overflow-hidden mt-12">
      ...
      <div className="relative z-10 h-full flex items-center">
        ...
        >
          ...
          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.98 }} className="inline-block bg-amber-600 hover:bg-amber-700 text-white font-semibold px-4 py-2 rounded shadow">
            Sign up for updates
          </motion.button>
        </motion.div>
      </div>
...
const Home = () => {
  ...
  const heroSlides = [
    ...
    {
      image: "https://images.unsplash.com/photo-1506806732259-39c2d0268443?q=80&w=1600&auto=format&fit=crop",
    ...
    <>
      ...
      <div className="container mx-auto px-4 mt-12">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-2xl md:text-3xl font-bold text-emerald-600">Available Fresh Crops</h2>
          <Link to="/all-crops" className="text-sm rounded-md bg-white border px-3 py-2 text-emerald-600 hover:bg-emerald-50">
            View All
          </Link>
        </div>
          ...
          {data.slice(0, 6).map((prev) => (
            ...
            >
              <div className="relative">
                ...
                <span className="absolute bottom-3 left-3 rounded-full bg-black/70 px-3 py-1 text-xs font-medium text-white">{prev?.location}</span>
              </div>
              ...
              <div className="flex flex-col gap-3 p-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600">Qty: {Intl.NumberFormat().format(prev?.quantity ?? 0)}</div>
                </div>
